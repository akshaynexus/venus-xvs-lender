...
/********************
 *   A lender plugin for LenderYieldOptimiser for any erc20 asset on venus (not bnb)
 *   Made by SamPriestley.com
 *   Modified for Venus,based on GenericCompound
 *
 ********************* */

contract GenericVenus is GenericLenderBase {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;

    uint256 private constant blocksPerYear = 10513334;//Rounded based on 3s per block on binance smart chain
    ...
    bool internal sellXVS;
    uint256 public minXvsToSell = 0.5 ether;
    ...
    constructor(
        ...
        sellXVS = address(want) != address(xvs);
    ...
    function _disposeOfXvs() internal {
        ...
        if (_xvs > minXvsToSell) {
            ...
            IUniswapV2Router02(pancakeRouter).swapExactTokensForTokens(_xvs, uint256(0), path, address(this), now);
        }
    }
